...
class MapTests: TestCase {
    ...
    func testEncodeMap16() {
        ...
        let encoded = try! MessagePack.encode(.map(makeMap(repeating: nil, count: Int(UInt8.max)+1)))
    ...
    func testEncodeMap32() {
        ...
        let encoded = try! MessagePack.encode(.map(makeMap(repeating: nil, count: Int(UInt16.max)+1)))
        assertEqual(encoded.prefix(3), expected.prefix(3))
        assertEqual(encoded.sorted(), expected.sorted())
    }
